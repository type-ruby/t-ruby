...
describe TRuby::Compiler do
  ...
  describe "#compile" do
    ...
    context "with return type validation" do
      ...
      it "raises TypeCheckError when yield argument type mismatches block signature" do
        Dir.mktmpdir do |tmpdir|
          input_file = File.join(tmpdir, "yield_type_mismatch.trb")
          File.write(input_file, <<~RUBY)
            def each(&block: Proc(Integer) -> void): void
      ...
      it "passes when yield argument types match block signature" do
        Dir.mktmpdir do |tmpdir|
          input_file = File.join(tmpdir, "yield_type_match.trb")
          File.write(input_file, <<~RUBY)
            def each(&block: Proc(String) -> void): void
              yield "hello"
              yield "world"
            end
          RUBY
